...
inline void FaceDetect::Release() {
    if (Net != nullptr) {
        delete Net;
        Net = nullptr;
    }
...
int FaceDetect::Init(const std::string &model_path){
    std::string param = model_path + "/retinaface.param";
    std::string bin = model_path + "/retinaface.bin";

    int a = Net->load_param(param.c_str());
    int b = Net->load_model(bin.c_str());
    std::cout <<"Gia tri : " << a << b << std::endl;
    return 0;
}

void FaceDetect::Detect(cv::Mat &bgr, std::vector <bbox> &boxes) {
...
inline bool FaceDetect::cmp(bbox a, bbox b) {
    if (a.s > b.s)
        return true;
    return false;
}

inline void FaceDetect::SetDefaultParams() {
    _nms = 0.4;
    _threshold = 0.6;
    _mean_val[0] = 104;
    _mean_val[1] = 117;
...
void FaceDetect::create_anchor(std::vector <box> &anchor, int w, int h) {
    anchor.clear();
...
void FaceDetect::create_anchor_retinaface(std::vector <box> &anchor, int w, int h) {
    anchor.clear();
...
void FaceDetect::nms(std::vector <bbox> &input_boxes, float NMS_THRESH) {
